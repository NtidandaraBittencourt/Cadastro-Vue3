<template>
  <div class="mb-4">
    ...
    <div v-if="type === 'radio' || type === 'checkbox'" class="mt-1">
      <label v-for="(option, index) in options" :key="index" class="mr-4 inline-flex items-center">
        <input
          :id="`${id}-${option.value}`"
          :type="type"
          :value="option.value"
    ...
    />
    <p v-if="localErrorMessage" class="text-sm text-red-600 mt-1">{{ localErrorMessage }}</p>
  </div>
</template>
